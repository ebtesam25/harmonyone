...
export default function Fitbit({route}){
  ...
  const _storeFitbitUser = async (value) => {
    try {
      const jsonValue = JSON.stringify(value)
      await AsyncStorage.setItem('@fitbit', jsonValue)
    } catch (e) {
      // saving error
    }
  }
  const _storeFitbitToken = async (value) => {
    try {
      const jsonValue = JSON.stringify(value)
      await AsyncStorage.setItem('@fitbit_token', jsonValue)
    } catch (e) {
      // saving error
    }
  }
  ...
  const _getActivity = (token) => {
    ...
    fetch("https://api.fitbit.com/1/user/66JB7Q/profile.json", requestOptions)
      .then(response => response.json())
      .then(result => {console.log(result); profile=result;setProfile(result);})
      .catch(error => console.log('error', error));
  ...
  React.useEffect(() => {
    if (response?.type === 'success'){
      ...
      _storeFitbitToken(access_token);
      _getActivity(access_token);
      const { user_id } = response.params;
      console.log(user_id);
      _storeFitbitUser(user_id);
      setUser(user_id);
