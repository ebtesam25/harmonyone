...
export default function Github({route}) {
  ...
  const _storeGithubUser = async (value) => {
    try {
      const jsonValue = JSON.stringify(value)
      await AsyncStorage.setItem('@github', jsonValue)
    } catch (e) {
      // saving error
    }
  }
  const _storeGithubToken = async (value) => {
    try {
      const jsonValue = JSON.stringify(value)
      await AsyncStorage.setItem('@github_token', jsonValue)
    } catch (e) {
      // saving error
    }
  }
  ...
  const _getUser = (token) => {
    ...
    fetch("https://api.github.com/user", requestOptions)
      ...
      .then(result => {console.log(result.login);setGithub(result.login);_getEvents(token,result.login);_storeGithubUser(result.login)})
      .catch(error => console.log('error', error));
  }
  ...
  return (
    <View style={{marginTop:'20%', paddingHorizontal:'5%'}}>
        ...
        <Text style={{fontSize:30, fontWeight:'bold', textAlign:'center'}}>Hey there!</Text>
        ...
        <TouchableOpacity onPress={()=>{navigation.navigate('Fitbit',{name:name,email:email,password:password})}}><View style={{flexDirection:'row', justifyContent:'space-around', alignContent:'center', borderRadius:10, width:'60%', paddingVertical:'2.5%', alignSelf:'center', backgroundColor:'#00B0B9'}}>
